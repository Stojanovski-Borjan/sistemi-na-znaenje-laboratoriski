...
if __name__ == '__main__':
    ...
    dolzina = int(len(dataset) * 0.75)      #postavuvanje na 75% od mnozhestvoto
    train_data = dataset[:dolzina]          #podelba na trening data
    test_data = dataset[dolzina:]           #podelba na test data

    docs = []
    labels = []
    for item in train_data:
        docs.append(item[0])            #Go stavame TEKSTOT od dokumentot
        labels.append(item[1])          #ja stavame KLASATA od dokumentot

    data, df, n, vocab = create_dataset(docs, labels)

    tree = build_tree(data)             #Gradime drvo preku create_dataset -> DATA

    TP, FP, TN, FN = 0, 0, 0, 0
    for item in test_data:
        test_vector = process_document(item[0], df, n, vocab)
        predviduvanje = max(classify(test_vector, tree).items(), key= lambda  x: x[1])[0]
        if item[1] == 'spam':
            if predviduvanje == item[1]:
                TP +=1
            else:
                FN +=1
        else:
    ...
    input_vector = process_document(test_comment, df, n, vocab)
    predviduvanje = max(classify(input_vector, tree).items(), key= lambda x: x[1])[0]
