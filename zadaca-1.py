...
podelba_prva = trainingData[:int(len(trainingData)*0.5)]
podelba_vtora = trainingData[int(len(trainingData)*0.5):]

gradenje_prvo = build_tree(podelba_prva, entropy)
gradenje_vtoro = build_tree(podelba_vtora, entropy)

klasifikacija_prva = classify(testCase, gradenje_prvo)
klasifikacija_vtora = classify(testCase, gradenje_vtoro)


sortiranje_prvo = sorted(klasifikacija_prva.items(), key = lambda x: x[1], reverse = True)[0][0]
sortiranje_vtoro = sorted(klasifikacija_vtora.items(), key = lambda x: x[1], reverse = True)[0][0]

if sortiranje_prvo != sortiranje_vtoro:
    print ("KONTRADIKCIJA")
else:
    print (sortiranje_prvo)