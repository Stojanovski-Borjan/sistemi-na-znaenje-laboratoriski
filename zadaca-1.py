...
def get_words_with_ignore(doc, words_to_ignore=None):
    # подели го документот на зборови и конвертирај ги во мали букви
    # па потоа стави ги во резултатот ако нивната должина е >2 и <20
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
           if words_to_ignore is not None and word.lower() not in words_to_ignore:
                words.add(word.lower())
    return words
...
def helper(doc):
    return get_words_with_ignore(doc, words_to_ignore)
if __name__ == "__main__":
    recenica = input()
    # vashiot kod tuka

# Klasifikacija NaiveBayes za trening
classificator = NaiveBayes(get_words)
classificator_ignore = NaiveBayes(helper)

#TRENING
for doc in train_data:
    classificator.train(doc[0], doc[1])
    classificator_ignore.train(doc[0], doc[1])
#Zavrshen Trening


# Klasifikacija na dokument
klasa_1 = classificator.classify_document(recenica)
klasa_2 = classificator_ignore.classify_document(recenica)
# Zavrshuva klasifikacija na dokument
print(klasa_1)
print(klasa_2)
