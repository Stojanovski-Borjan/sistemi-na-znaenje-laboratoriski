...
podelba_roach = [item for item in data if item[-1] == "Roach"][:40]
podelba_pike = [item for item in data if item[-1] == "Pike"][:40]

nova_data = podelba_pike + podelba_roach

gradenje_nova_data = build_tree(nova_data, entropy)

klasifikacija_nova_data = classify(test_case, gradenje_nova_data)

predviduvanje_klasa = sorted(klasifikacija_nova_data.items(), key = lambda x: x[1], reverse=True)[0][0]

print (predviduvanje_klasa)