...
classificator1 = NaiveBayes(get_words)
classificator2 = NaiveBayes(get_words)

training_classificator1 = train_data[:int(len(train_data)*0.4)]
training_classificator2 = train_data[int(len(train_data)*0.4):int(len(train_data)*0.8)]

testing_classificator = train_data[int(len(train_data)*0.8):]

for doc in training_classificator1:
    classificator1.train(doc[0], doc[1])

for doc in training_classificator2:
    classificator2.train(doc[0], doc[1])

brojac = 0

for doc in testing_classificator:
    klasa1 = classificator1.classify_document(doc[0])
    klasa2 = classificator2.classify_document(doc[0])
    if klasa1 == klasa2 and klasa1 == doc[1]:
        brojac += 1
        print(brojac / len(testing_classificator))

rezultat1 = classificator1.classify_document(sample)
rezultat2 = classificator2.classify_document(sample)
