...
if __name__ == "__main__":
    ...
    # podelba na drva TRENIRACKO I TESTIRACKO
    treniracko = [train_data[item] for item in range(len(train_data)) if item not in test_indices]
    testiracko = [train_data[item] for item in range(len(train_data)) if item in test_indices]

    # Procentualna podelba na treniracko drvo
    drvo_1 = treniracko[:int(len(treniracko) * 0.5)]
    drvo_2 = treniracko[int(len(treniracko) * 0.5):]

    # GRADENJE NA DRVA
    tree_1 = build_tree(drvo_1, entropy)
    tree_2 = build_tree(drvo_2, entropy)

    brojac_1 = 0
    brojac_2 = 0
    for item in testiracko:
        # KLASIFIKACIJA
        k1 = max(classify(item, tree_1).items())[0]
        # ZEMANJE NA TOCHNATA KLASA
        # k1 = sorted(klasa_1.items(), key=lambda x: x[1], reverse=True)[0][0]
        if k1 == item[-1]:
            brojac_1 += 1

        # KLASIFIKACIJA
        klasa_2 = classify(item, tree_2)
        # ZEMANJE NA TOCHNATA KLASA
        k2 = sorted(klasa_2.items(), key=lambda x: x[1], reverse=True)[0][0]
        if k2 == item[-1]:
            brojac_2 += 1

    tocnost_1 = brojac_1 / len(testiracko)
    tocnost_2 = brojac_2 / len(testiracko)

    print("Tochnost na drvoto 1: " f'{tocnost_1}')
    print(f'Tochnost na drvoto 2: {tocnost_2}')
    # print (tocnost_2)
    if tocnost_1 == tocnost_2:
        print("Drvata imaat ista tochnost")
    elif tocnost_1 > tocnost_2:
