...
def get_words_emoticons(doc, emoticons=None):
    """Поделба на документот на зборови. Стрингот се дели на зборови според
    празните места и интерпукциските знаци
...
if __name__ == '__main__':
    ...
    classificator2 = NaiveBayes(helpers)

    for item in data:
        classificator1.train(item[1], item[0])
        classificator2.train(item[1], item[0])

    klasa1 = classificator1.classify_document(testiracko[1])
    klasa2 = classificator2.classify_document(testiracko[1])

    print(testiracko[1])
    print('Vistinska klasa: ' f'{testiracko[0]}')
    print('Klasa predvidena so Naiven Baes (bez emotikoni): ' f'{klasa1}')
    print('Klasa predvidena so Naiven Baes (so emotikoni): ' f'{klasa2}')
