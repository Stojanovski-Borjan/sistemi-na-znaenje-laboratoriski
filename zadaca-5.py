...
data = [('A very, very, very slow-moving, aimless movie about a distressed, drifting young man.', 0),
        ...
        1), ("I don't think you will be disappointed.", 1), (
        ...
        1), ('Regrettably, the film fails.', 0),
        (
...
def get_words_to_include(doc, words_to_include=None):
    """Поделба на документот на зборови. Стрингот се дели на зборови според
    празните места и интерпукциските знаци
    ...
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            if words_to_include is not None and word.lower() in words_to_include:
                words.add(word.lower())
    return words

def helper(doc):
    return get_words_to_include(doc, words_to_include)

if __name__ == '__main__':
    comment = input()

classificator1 = NaiveBayes(get_words)
classificator2 = NaiveBayes(helper)

for doc in data:
    classificator1.train(doc[0], doc[1])
    classificator2.train(doc[0], doc[1])

klasa1 = classificator1.classify_document(comment)
klasa2 = classificator2.classify_document(comment)


print('Klasa predvidena so site zborovi: ' f'{klasa1}')
print('Klasa predvidena so samo kluchni zborovi: ' f'{klasa2}')
