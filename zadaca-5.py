...
def print_tree(tree, i, indent=''):
    ...
    if tree.results:
        ...
        # Се печатат True гранките, па False гранките
        print(indent + 'T-> ', end='')
        print_tree(tree.tb,i+1, indent + '  ')
        print(indent + 'F-> ', end='')
        print_tree(tree.fb,i+1, indent + '  ')
...
if __name__ == "__main__":
    ...
    # Delenje na drva
    drvo_1 = training_data[:int(len(training_data) * 0.5)]
    drvo_2 = training_data[int(len(training_data) * 0.5):]

    # GRADENJE NA DRVA
    trening_drvo_1 = build_tree(drvo_1, entropy)
    trening_drvo_2 = build_tree(drvo_2, entropy)

    # KLASIFIKACIJA NA DRVATA
    classification_drvo_1 = classify(testCase, trening_drvo_1)
    classification_drvo_2 = classify(testCase, trening_drvo_2)

    # ZEMANJE NA TOCHNATA KLASA
    res_1 = sorted(classification_drvo_1.items(), key=lambda x: x[1], reverse=True)[0][0]
    res_2 = sorted(classification_drvo_2.items(), key=lambda x: x[1], reverse=True)[0][0]

    print_tree(trening_drvo_1, 0)
    print_tree(trening_drvo_2, 0)
