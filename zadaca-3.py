...
# Gradenje na klasifikator
classificator = NaiveBayes(get_words)
# END Gradenje na klasifikator

# Podelba na trening i test klasifikatori
training_classificator = train_data[:int(len(train_data)*0.7)]

testing_classificator = train_data[int(len(train_data)*0.7):]
# END Podelba na trening i test klasifikatori

# Testiranje na klasifikator
for doc in training_classificator:
    classificator.train(doc[0], doc[1])
# END Testiranje na klasifikator


# Presmetka na tochnost
brojac = 0
for doc in testing_classificator:
    klasa = classificator.classify_document(doc[0])
    if klasa == doc[1]:
        brojac += 1

print(brojac/ len(testing_classificator))
# END Presmetka na tochnost

klasa_pecati = classificator.classify_document(sample)
print(klasa_pecati)