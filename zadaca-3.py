...
if __name__ == '__main__':
    ...
    data = []                                           #kreiram prazna data
    for item in dataset:                                #go izminuvam cel DATASET
        data.append(item[0])                            # Vo DATA gi dodavam site dokumenti (TEKSTOVI) od dataset
    # data = [x[0] for x in dataset]                    ISTO E SO GORNI TRI REDA

    rez = rank_documents(comment, data, cosine)         #vrakja najslichni dokumenti za daden dokument spored zadadeno baranje COSINE, vrakja vo DECIMALI
    rez = rez[:n]                                       #ogranichuvam kolku max dokumenti da ima vo REZ, ogranichuvam preku input - n

    brojacSpam = 0
    brojacHam = 0

    rez = [item for item in rez if item[0] > 0]         #vo rez zacuvaj gi site dokumenti koi se pogolemi od 0
    n = len(rez)

    for item in rez:
        if dataset[item[1]][1] == "spam":                           #proveruva dali klasata e spam, ako e zgolemi brojach na SPAM
            brojacSpam += 1
        else:                                                       #vo sprotivno zgolemi brojach na HAM
            brojacHam += 1

    klasa = ""
    if brojacSpam >= brojacHam:                                     #dokolku SPAM >= HAM pechati spam, vo sprotivno pechati ham
        klasa = "spam"
    else:
        klasa = "ham"

    if klasa == "spam":                                             #Dokolku klasata e spam pechati kolku pogodoci od kolku ima SPAM
        print(f'Predvidenata klasa e {klasa} so {brojacSpam}/{n} glasovi.')
    else:                                                           #Dokolku klasata e ham pechati kolku pogodoci od kolku ima HAM
        print(f'Predvidenata klasa e {klasa} so {brojacHam}/{n} glasovi.')