...
if __name__ == '__main__':
    ...
    # PODELBA NA MNOZHESTVO
    mnozhestvo1 = dataset[:int(len(dataset) * 0.3)]
    mnozhestvo2 = dataset[int(len(dataset) * 0.3):int(len(dataset) * 0.6)]
    mnozhestvo3 = dataset[int(len(dataset) * 0.6):]
    # ZAVRSHUVA SO PODELBA NA MNOZHESTVO

    # GRADI I TRENIRA DRVO
    tree1 = build_tree(mnozhestvo1, entropy)
    tree2 = build_tree(mnozhestvo2, entropy)
    tree3 = build_tree(mnozhestvo3, entropy)
    # ZAVRSHUVA SO GRADENJE I TRENIRANJE NA DRVO

    # KLASIFICIRA
    glas1 = max(classify(test_case, tree1).items())[0]  # class,value
    glas2 = max(classify(test_case, tree2).items())[0]
    glas3 = max(classify(test_case, tree3).items())[0]
    # ZAVRSHUVA SO KLASIFICIRANJE

    # RECNIK, ZADAVA 0 NA KEY
    glasovi = {0: 0, 1: 0, 2: 0}
    # ZAVRSHUVA RECNIK

    # PREGLEDUVA I ZGOLEMUVA GLASOVI
    glasovi[glas1] += 1
    glasovi[glas2] += 1
    glasovi[glas3] += 1
    # ZAVRSHUVA SO PREGLEDUVNJE I ZGOLEMUVA GLASOVI

    # SORTIRA OD NAJMNOGU KON NAJMALCE GLASOVI
    sortirana = list(sorted(glasovi.items(), key=lambda x: x[1], reverse=True))
    # ZAVRSHUVA SO SORTIRANJE OD NAJMNOGU KON NAJMALCE GLASOVI

    # PECATI GLASOVI SPORED KLASA SO FUNKCIJATA DICT()
    print("Glasovi:", dict(glasovi))  # rechnik dictionary
    # ZAVRSUVA SO PECATENJE GLASOVI

    # SPOREDBA
    zacuvaj = sortirana[0][0]
    if sortirana[0][1] == sortirana[1][1]:
        print("Predvidena klasa: unknown")
    else:
        print("Predvidena klasa:", zacuvaj)