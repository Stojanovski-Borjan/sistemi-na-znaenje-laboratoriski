...
if __name__ == "__main__":
    ...
    # Vashiot kod tuka
    golemina = int(len(dataset) * 0.8)
    podatochno_mnozhestvo1 = dataset[:golemina]
    testiracko_mnozestvo1 = dataset[golemina:]

    podatochno_mnozhestvo2 = [extract_column(row, column_ind) for row in podatochno_mnozhestvo1]
    testiracko_mnozestvo2 = [extract_column(row, column_ind) for row in testiracko_mnozestvo1]

    drvo1 = build_tree(podatochno_mnozhestvo1)
    drvo2 = build_tree(podatochno_mnozhestvo2)
    fc = 0
    sc = 0
    for test in testiracko_mnozestvo1:
        rezultat1 = max(classify(test, drvo1).items())[0]
        if rezultat1 == test[-1]:
            fc += 1
    for x in testiracko_mnozestvo2:
        rezultat2 = max(classify(x, drvo2).items())[0]
        if rezultat2 == x[-1]:
            sc += 1
    tochnost1 = fc / len(testiracko_mnozestvo1)
    tochnost2 = sc / len(testiracko_mnozestvo2)

    print('Tochnost so prvoto drvo na odluka: ' + str(tochnost1))
    print('Tochnost so vtoroto drvo na odluka: ' + str(tochnost2))
